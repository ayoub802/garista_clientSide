...
// };
  ...
  return (
   ...
   <div className="container m-0.5">
     <div className="flex flex-col">
       ...
       <div className="grid grid-cols-2  flex-wrap gap-[1rem]"> {/* Utilisation de flex-wrap pour permettre le retour à la ligne */}
         {data.map((item, index) => (
           <div key={item.id} className="w-full md:w-1/2" style={{ borderRightRadius: '5px solid white' }}> {/* Ajustement de la largeur de chaque élément */}
             <div className="md:max-w-3xl grid menu-item gap-4 items-center">
               <div className="overflow-hidden flex justify-center"> {/* Ajustement de la hauteur */}
                 <img src={item.image} className="w-[12rem] bg-cover rounded-3xl" alt="burger" /> {/* Ajustement de la largeur */}
               </div>
               <div className="flex flex-col justify-center ">
                 <h3 className="text-sm text-center mb-3">{item.title}</h3>
                
                 <div className="flex justify-between pb-1 text-xs md:text-sm lg:text-base xl:text-lg text-gray-600">
                   <p className="mb-0.5 text-sm text-white">{item.price}</p>
                   <button
                     type="button"
             ...
             {/* {index !== data.length - 1 && <hr className="w-full bg-white" />}  */}
             <hr className="w-full bg-white" />
           </div>
         ))}
       </div>
 ...
 {isModalOpen && selectedItem && (
  <Modal selectedItem={selectedItem} toggleModal={toggleModal} />
)}
